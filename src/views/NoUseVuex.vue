...
export default class HomeView extends Vue {
  ...
  async saveForm(data: IForm): Promise<unknown> {
    try {
      const result: any = await new LocalStorageService().setData('form', data);

      this.localForm = result.data;
  ...
  async getForm(): Promise<void> {
    try {
      const result: any = await new LocalStorageService().getData('form');

      this.localForm = result;
    } catch (error: any) {
